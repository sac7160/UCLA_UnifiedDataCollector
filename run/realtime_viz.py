...
def _mic_callback(indata, frames, time_info, status):
    ...
    if _mic_band_sos is not None:
        ...
        if _noise_floor is None or _envelope < _noise_floor:
            _noise_floor = _envelope   # track downward immediately
        elif not _touch_on_state:
            # Only creep the floor UP while we're not already confirmed ON.
            # Without this guard, a long/continuous touch (very plausible on
            # acrylic, which is quieter so touches often run right up
            # against a low threshold) would keep _envelope elevated for
            # seconds at a time, and the floor would slowly chase it upward
            # (tau=2s) — shrinking the dB gap between "touching" and
            # "floor" the longer a touch lasts, right when you need that
            # gap the most. The floor should only ever adapt to genuine
            # between-touches ambient noise.
            coef_floor = float(np.exp(-block_dt / FLOOR_RISE_TAU_SEC))
            _noise_floor = coef_floor * _noise_floor + (1.0 - coef_floor) * _envelope   # creep upward slowly
...
class LiveDashboard(QtWidgets.QMainWindow):
    ...
    def _sync_touch_thresholds(self):
        ...
        # The metric is dB *above the noise floor's own recent minimum*, so
        # during genuine quiet it structurally sits close to 0dB and rarely
        # sustains a value much below that. An off-threshold at or below
        # ~0.3dB is close to unreachable — the state will confirm ON but
        # then effectively never confirm OFF again (looks like it's stuck).
        # Not auto-corrected here (your call), but flagged loudly.
        if off <= 0.3:
...
def main():
    ...
    parser.add_argument('--touch-band-low', type=float, default=TOUCH_BAND_LOW_HZ,
                         help=f'lower edge of the band-pass filter used for touch detection, Hz '
                              f'(default {TOUCH_BAND_LOW_HZ:.0f}). Validated on wood — a different '
                              f'surface material (e.g. acrylic) may put its friction-sound energy '
                              f'in a different band; check the surface-mic spectrogram panel while '
                              f'touching to see where the energy actually concentrates and narrow/'
                              f'shift this range to match, before tightening the threshold.')
    parser.add_argument('--touch-band-high', type=float, default=TOUCH_BAND_HIGH_HZ,
                         help=f'upper edge of the band-pass filter used for touch detection, Hz '
                              f'(default {TOUCH_BAND_HIGH_HZ:.0f}).')
    parser.add_argument('--list-devices', action='store_true')
    args = parser.parse_args()
